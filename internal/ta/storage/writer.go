...
func turkishQuantAnalysis(q analysis.QuantAnalysis) map[string]any {
	return map[string]any{
		...
		"getiri": map[string]any{
			...
			"getiri_5g_yuzde":                q.Return.Return5DPct,
			"getiri_20g_yuzde":               q.Return.Return20DPct,
			"getiri_60g_yuzde":               q.Return.Return60DPct,
			"getiri_120g_yuzde":              q.Return.Return120DPct,
			"getiri_252g_yuzde":              q.Return.Return252DPct,
			"ortalama_gunluk_getiri_yuzde":   q.Return.MeanDailyReturnPct,
			"yilliklandirilmis_getiri_yuzde": q.Return.AnnualizedReturnPct,
			"cagr_yuzde":                     q.Return.CAGRPct,
			"pozitif_seans_orani_yuzde":      q.Return.PositiveSessionRatioPct,
		},
		"risk": map[string]any{
			"gunluk_volatilite_yuzde":      q.Risk.DailyVolatilityPct,
			"yillik_volatilite_yuzde":      q.Risk.AnnualizedVolatilityPct,
			"asagi_yonlu_volatilite_yuzde": q.Risk.DownsideVolatilityPct,
			"historik_var_95_yuzde":        q.Risk.HistoricalVaR95Pct,
			"historik_cvar_95_yuzde":       q.Risk.HistoricalCVaR95Pct,
			"parametrik_var_95_yuzde":      q.Risk.ParametricVaR95Pct,
			"maksimum_dusus_kaybi_yuzde":   q.Risk.MaxDrawdownLossPct,
			"guncel_dusus_kaybi_yuzde":     q.Risk.CurrentDrawdownLossPct,
			"sharpe":                       q.Risk.SharpeRatio,
			"sortino":                      q.Risk.SortinoRatio,
			"calmar":                       q.Risk.CalmarRatio,
			"carpiklik":                    q.Risk.Skewness,
			"basiklik_fazlasi":             q.Risk.ExcessKurtosis,
			"volatilite_rejimi":            q.Risk.VolatilityRegime,
			"100k_icin_1g_var_riski":       q.Risk.RiskBudgetOneDayVaRPer100K,
		},
		"benchmark": map[string]any{
			"sembol":                q.Benchmark.Symbol,
			"var":                   q.Benchmark.Available,
			"hata":                  q.Benchmark.Error,
			"goreli_guc_20g_yuzde":  q.Benchmark.RelativeStrength20Pct,
			"goreli_guc_60g_yuzde":  q.Benchmark.RelativeStrength60Pct,
			"beta_60":               q.Benchmark.Beta60,
			"alpha_60_yillik_yuzde": q.Benchmark.Alpha60AnnualPct,
			"korelasyon_60":         q.Benchmark.Correlation60,
		},
		"karar": map[string]any{
			"skor":         q.Decision.Score,
			"risk_skoru":   q.Decision.RiskScore,
			"getiri_skoru": q.Decision.ReturnScore,
			"etiket":       q.Decision.Label,
			"uygunluk":     q.Decision.Suitability,
			"ozet":         q.Decision.Summary,
			"gecenler":     reportLabels(q.Decision.Passes),
...
func turkishStatEconomicAnalysis(s analysis.StatEconomicAnalysis) map[string]any {
	return map[string]any{
		"hesaplandi":   s.Computed,
		"durum":        s.Status,
		"ornek_sayisi": s.SampleCount,
		"bilesik_skor": s.CompositeScore,
		"veri_butunlugu": map[string]any{
			"skor":               s.DataIntegrity.Score,
			"durum":              s.DataIntegrity.Status,
			"mum_sayisi":         s.DataIntegrity.CandleCount,
			"tekrarli_tarih":     s.DataIntegrity.DuplicateDates,
			"kronoloji_hatasi":   s.DataIntegrity.ChronologyViolations,
			"fiyat_kural_hatasi": s.DataIntegrity.PriceRuleViolations,
			"getiri_outlier":     s.DataIntegrity.ReturnOutliers,
			"hacim_outlier":      s.DataIntegrity.VolumeOutliers,
			"uyarilar":           reportLabels(s.DataIntegrity.Warnings),
		},
		"faktor_modeli": map[string]any{
			"skor":                  s.FactorModel.Score,
			"beta_60":               s.FactorModel.MarketBeta60,
			"alpha_60_yillik_yuzde": s.FactorModel.MarketAlpha60AnnualPct,
			"korelasyon_60":         s.FactorModel.MarketCorrelation60,
			"goreli_guc_60_yuzde":   s.FactorModel.RelativeStrength60Pct,
			"momentum_skoru":        s.FactorModel.MomentumScore,
			"kalite_skoru":          s.FactorModel.QualityProxyScore,
			"deger_skoru":           s.FactorModel.ValueProxyScore,
			"dusuk_vol_skoru":       s.FactorModel.LowVolatilityScore,
			"likidite_skoru":        s.FactorModel.LiquidityScore,
			"faktorler":             s.FactorModel.Factors,
		},
		"rejim_modeli": map[string]any{
			"skor":                           s.Regime.Score,
			"trend_rejimi":                   s.Regime.TrendRegime,
			"volatilite_rejimi":              s.Regime.VolatilityRegime,
			"drawdown_rejimi":                s.Regime.DrawdownRegime,
			"ewma_yillik_vol_yuzde":          s.Regime.EWMAVolatilityAnnualPct,
			"garch_tahmini_vol_yuzde":        s.Regime.GARCHVolatilityForecastPct,
			"volatilite_kumelenme_skoru":     s.Regime.VolatilityClusteringScore,
			"sonraki_pozitif_olasilik_yuzde": s.Regime.NextPositiveProbabilityPct,
		},
		"stres": map[string]any{
			"skor":                      s.TailStress.Score,
			"var_95_yuzde":              s.TailStress.VaR95Pct,
			"cvar_95_yuzde":             s.TailStress.CVaR95Pct,
			"asiri_kuyruk_kaybi_yuzde":  s.TailStress.ExtremeTailLossPct,
			"en_kotu_seans_kaybi_yuzde": s.TailStress.WorstSessionLossPct,
			"stres_senaryolari":         s.TailStress.StressScenarios,
		},
		"makro_duyarlilik": s.MacroSensitivity,
		"finansal_kalite":  s.FinancialQuality,
		"likidite":         s.Liquidity,
		"validasyon":       s.Validation,
		"uyarilar":         reportLabels(s.Warnings),
	}
}
